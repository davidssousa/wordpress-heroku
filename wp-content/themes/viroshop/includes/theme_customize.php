...
function virohop_styles() {
	...
	if ($options['viro_background_image']) {
		$custom_css .= "body {
							background: $options[viro_background_color] url($options[viro_background_image]);
						}";
	} else {
		$custom_css .= "body {
							background: $options[viro_background_color] url( ". get_template_directory_uri() . "/images/patterns/$options[viro_background_pattern]);
						}";
	}
